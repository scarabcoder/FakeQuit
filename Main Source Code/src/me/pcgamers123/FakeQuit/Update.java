...
public class Update {
    ...
    private Update.UpdateResult result = Update.UpdateResult.SUCCESS; // Used for determining the outcome of the update process
     ...
     */
    public Update(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        final File updaterFile = new File(pluginFile, "FakeQuitUpdater");
        final File updaterConfigFile = new File(updaterFile, "Update.yml");

        this.config.options().header("DO NOT TOUCH THE 'api-key'! IT WILL MESS UP THE UPDATE SYSTEM IF YOU DO!" +  '\n'
        + "You can Enable or Disable the Update Future by changing the 'disable' to true!");
        this.config.addDefault("api-key", "70536");
        ...
        if (key.equalsIgnoreCase("70536") || key.equals("")) {
            key = null;
        }
    ...
    private void saveFile(File folder, String file, String link) {
        ...
        try {
            ...
            final byte[] data = new byte[Update.BYTE_SIZE];
            int count;
            if (this.announce) {
            ...
            Bukkit.reload();
            Bukkit.broadcastMessage(ChatColor.GREEN + "FakeQuit has been Updated! Reload Complete!");
        }
    }
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (entry.isDirectory()) {
                    ...
                    int b;
                    final byte buffer[] = new byte[Update.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, Update.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, Update.BYTE_SIZE)) != -1) {
                    ...
                    if (this.pluginFile(dFile.getName())) {
                        ...
                        for (final File cFile : dFile.listFiles()) // Loop through all the files in the new dir
                        {
                            ...
                            if (!found) {
                                ...
                                cFile.renameTo(new File(oFile.getCanonicalFile() + File.separator + getLatestName()));
                            } else {
    ...
    private boolean hasTag(String version) {
        for (final String string : Update.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            conn.addRequestProperty("User-Agent", Update.USER_AGENT);
            ...
            this.versionType = (String) ((JSONObject) array.get(array.size() - 1)).get(Update.TYPE_VALUE);
            this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1)).get(Update.VERSION_VALUE);
